...
typedef struct VideoState {
    ...
    uint8_t audio_buf[(MAX_AUDIO_FRAME_SIZE * 3) / 2];    /** 解码后的音频数据 */
    unsigned int audio_buf_size;                    /** 解码后音频数据总size */
    unsigned int audio_buf_index;                /** 当前已使用偏移 */
    AVFrame audio_frame;                    /** 存放解码后的一帧音频 */
    AVPacket audio_pkt;                    /** 存放当前需要解码的一个音频包 */
    uint8_t *audio_pkt_data;                /** 初始化为指向一个AVPacket的音频帧数据 */
    int audio_pkt_size;                    /** 初始化为一个AVPacket音频帧的数据长度 */
    int audio_hw_buf_size;                /** 用来干啥的??? */ 
    double audio_diff_cum;                    /** used for AV difference average computation */
    double audio_diff_avg_coef;                /** 固定值c,用于加权平均 */
    double audio_diff_threshold;
    int audio_diff_avg_count;                /** 每AUDIO_DIFF_AVG_NB帧检测一下同步情况，计数器 */
    double frame_timer;
    double frame_last_pts;
    double frame_last_delay;

    /**
     如果某一帧没有pts信息，就用这个值设置为该帧的pts
     */
        ...
        if(is->audio_diff_avg_count < AUDIO_DIFF_AVG_NB) {
            ...
            if(fabs(avg_diff) >= is->audio_diff_threshold) {
                ...
                if(wanted_size < samples_size) {
                    ...
                    //nb = (samples_size - wanted_size); //源代码，怀疑这里写错了
...
int audio_decode_frame(VideoState *is, double *pts_ptr) 
{
    ...
    for(;;) {
        while(is->audio_pkt_size > 0) {
            int got_frame = 0;
            /**
             一个AVPicture数据包含多个音频帧的编码后信息，一次解码一帧音频帧，故分多次解码
             返回值len1表示，一次解码了len1字节的原始编码音频数据，len1的音频编码数据解码后长度为data_size
             */
            ...
            *pts_ptr = pts;
            n = 2 * is->audio_st->codec->channels;
            /**
             每个音频AVPacket里又有多帧音频AVFrame，计算每个AVFrame的pts值
             */
        ...
        /**
         初始化audio_clock为一个音频AVPacket的pts值
         */
...
void audio_callback(void *userdata, Uint8 *stream, int len) 
{
    ...
    while(len > 0) {
        if(is->audio_buf_index >= is->audio_buf_size) {
            ...
            /**
             audio_decode_frame函数返回一个音频帧数据长度（解码后）
             */
            ...
            if(audio_size < 0) {
                ...
                /**
                 同步音频到其他时钟, 假设同步到视频，假设此时音频比视频快，
                 假设视频播放到第7秒，而音频播放到第10秒, 那就让第10秒的音频连续播放3秒
                 这样当视频播放到第11秒的时候 音频也能刚好播放到第11秒（连续播放的实现是重复最后一个采样）

                 如果音频比视频慢，视频播放到7秒，音频播放到5秒，截断相关采样，让音频接下来6, 7, 8秒要播放的采样
                 在第6秒全部播放完，这样在视频播放的第9秒的时候，音频也刚好播放到第9秒
                 */
...
void video_refresh_timer(void *userdata) 
{
    ...
    if(is->video_st) {
        if(is->pictq_size == 0) {
            ...
            /**
             两个视频帧的pts差值，在(0s, 1.0s)这个区间内，如果帧率是25fps, 那么这个值应该在40ms,即0.04s附近波动
             如果不在上述区间，则把这个delta pts设置成与上一个相同
             */
            ...
            /**
             视频跟自己同步就不需要改变delay, 如果是同步到音频，或者wallclock就需要判断是否在同步阀值内
             是否需要改变delay的值
             */
            if(is->av_sync_type != AV_SYNC_VIDEO_MASTER) {
                ref_clock = get_master_clock(is);
            ...
            /**
             在stream_component_open处理视频的时候有初始化一个值frame_timer, is->frame_timer = (double)av_gettime() / 1000000.0;
             我们把这个时间设为time_a, 这个认为是视频最开始的时间，现在假设最开始的情况是开始播放第一帧视频，delay秒后播放第一帧视频
             即要在wallclock的 time_a + delay秒开始播放第一帧，设这个时间为time_c, 当前时间是time_b. 时间关系如下所示
             time_a ------ time_b ------------------------------------------------ time_c
             如果我们在time_b还延迟delay秒播放第一帧就不准确了, 所以需要av_gettime函数来校正delay时间获取actual_delay
             */
...
int decode_thread(void *arg) 
{
    ...
    /**
     不理解这个io_context到底怎么回事？
     只在这里初始化，并未关联到pFormatCtx
     */
