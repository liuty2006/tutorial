...
int video_thread(void *arg) {
  ...
  for(;;) 
  {
    ...
    // Decode video frame
    avcodec_decode_video2(is->video_st->codec, pFrame, &frameFinished,packet);//解码成帧
...
int decode_thread(void *arg) {
  ...
  for(;;) 
  {
    ...
    if(av_read_frame(is->pFormatCtx, packet) < 0) //读数据
    {
    ...
    if(packet->stream_index == is->videoStream) {//判断是否视频流
...
int main(int argc, char *argv[]) {
  ...
  av_strlcpy(is->filename, "out.mp4", 1024);
  //av_strlcpy(is->filename, "test.flv", 1024);
